    ...
    public class Program
    {
        ...
        public void createProcessFile (string fileName)
        {
            ...
            for (int j = 0; j < elements.Count; ++j)
            {
                ...
                if (elements[j].ParentNode.Name == Config.STAGE_TAG && elements[j].ParentNode.FirstChild.Name == Config.SUBSHEET_ID_ATTRIBUTE && elements[j].ParentNode.ParentNode.ParentNode.Name == Config.PROCESS_TAG)
                {
                    row = results.NewRow();
                ...
                if (elements[j].ParentNode.FirstChild.Name == Config.NARRATIVE_TAG)
                {
                    row = results.NewRow();
                    row[Config.TYPE_HEADER] = elements[j].Attributes[Config.TYPE_ATTRIBUTE].Value;
                    row[Config.DETAILS_HEADER] = elements[j].Attributes[Config.DETAIL_ATTRIBUTE].Value;
